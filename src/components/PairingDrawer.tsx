...
export const PairingDrawer: React.FC<PairingDrawerProps> = ({
    ...
    return (
        ...
        >
            <div
                className="w-full md:w-[var(--drawer-width)] bg-[var(--background)] h-full shadow-[var(--drawer-shadow)] flex flex-col animate-in slide-in-from-right duration-300 rounded-l-[var(--radius-drawer)]"
            ...
            >
                ...
                <header className="p-6 border-b border-[var(--border-subtle)] flex justify-between items-center bg-white">
                    <div>
                        <h2 className="text-xl font-bold flex items-center gap-2">
                            <ChefHat className="text-[var(--turmeric)]" size={20} />
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-8">
                    ...
                    <section>
                        <h3 className="text-[10px] font-black uppercase tracking-widest text-[var(--turmeric)] mb-4 flex items-center gap-2">
                            <History size={14} />
                            Frequently Paired With
                        </h3>
                        {loading ? (
                            <div className="flex justify-center py-4">
                                <div className="animate-spin h-5 w-5 border-2 border-[var(--turmeric)] border-t-transparent rounded-full" />
                            </div>
                    ...
                    <section className="space-y-4">
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full pl-10 pr-4 py-3 rounded-xl border border-[var(--border-subtle)] bg-white text-sm focus:ring-2 focus:ring-[var(--turmeric)] focus:border-transparent transition-all"
                                value={searchQuery}
                ...
                <footer className="p-6 bg-white border-t border-[var(--border-subtle)]">
                    <button
                        onClick={onClose}
                        className="w-full btn-premium py-4 shadow-lg flex justify-center items-center gap-2 bg-[var(--text-main)] text-white rounded-[var(--radius-card)] font-bold transition-transform active:scale-95"
                    >
...
const RecipePairCard: React.FC<{
    ...
    <button
        onClick={onToggle}
        className={`w-full text-left p-4 rounded-[var(--radius-card)] border transition-all flex justify-between items-center group
      ${isSelected
                ? 'border-[var(--turmeric)] bg-[var(--turmeric)]/5 shadow-sm'
                : 'border-[var(--border-subtle)] bg-white hover:border-[var(--turmeric)]/50 hover:shadow-md'}`}
    >
        <div>
            <h4 className={`font-bold text-sm ${isSelected ? 'text-[var(--beetroot)]' : 'text-[var(--text-main)]'}`}>
                {recipe.name}
            </h4>
      ...
      ${isSelected ? 'bg-[var(--turmeric)] text-white' : 'bg-gray-50 border border-gray-200 text-gray-300 group-hover:border-[var(--turmeric)]'}`}>
