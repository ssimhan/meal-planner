...
export default function SwapConfirmationModal({ day1, day2, onConfirm, onCancel }: SwapConfirmationModalProps) {
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white rounded-xl shadow-2xl max-w-sm w-full p-6 transform transition-all animate-in zoom-in-95 duration-200">
                ...
                <div className="flex gap-3">
                    <button
                        onClick={onCancel}
                        className="flex-1 px-4 py-2 rounded-lg border border-[var(--border-subtle)] text-[var(--text-secondary)] font-medium hover:bg-gray-50 transition-colors"
                    >
                        Cancel
                    </button>
                    <button
                        onClick={onConfirm}
                        className="flex-1 px-4 py-2 rounded-lg bg-[var(--accent-sage)] text-white font-medium hover:brightness-95 transition-all shadow-md active:scale-95"
                    >
                        Confirm Swap
                    </button>
                </div>
